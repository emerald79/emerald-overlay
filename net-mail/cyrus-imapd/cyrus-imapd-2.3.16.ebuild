...
IUSE="idled kerberos mysql nntp pam replication +sieve snmp ssl tcpd"
...
src_compile() {
	...
	if use mysql; then
		...
		myconf="${myconf} --with-mysql-incdir=/usr/include/mysql"
		myconf="${myconf} --with-mysql-libdir=/usr/$(get_libdir)/mysql"
	fi
